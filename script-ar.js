...
const weddingConfig = {
  ...
  venueAddress: 'سويس كلوب، القاهرة، مصر',
  ...
  // Same endpoint as the English page, so both versions land in one guest list.
  ...
  rsvpStorageKey: 'omar_rewan_rsvps_ar',
};
...
function buildGoogleCalendarUrl() {
  const details = [
    'يشرفنا تواجدكم معانا.',
    `المكان: ${weddingConfig.venueName}`,
    `الموقع على الماب: ${weddingConfig.venueMapsUrl}`,
  ].join('\n');

  const params = new URLSearchParams({
    action: 'TEMPLATE',
    text: `فرح ${weddingConfig.coupleNames}`,
...
function buildIcsFile() {
  ...
  const uid = `omar-rewan-ar-${Date.now()}@wedding-invitation`;
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Omar and Rewan Wedding//AR',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:${uid}`,
    ...
    `DTSTART:${start}`,
    `DTEND:${end}`,
    `SUMMARY:فرح ${weddingConfig.coupleNames}`,
    `LOCATION:${weddingConfig.venueAddress}`,
    `DESCRIPTION:المكان: ${weddingConfig.venueName}\\nالموقع على الماب: ${weddingConfig.venueMapsUrl}`,
    'END:VEVENT',
    'END:VCALENDAR',
  ];
...
function initRsvpForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!payload.fullName) {
      status.textContent = 'من فضلك اكتبوا اسمكم قبل ما تبعتوا.';
    ...
    try {
      ...
      status.textContent = 'مكانكم اتحجز. مش قادرين نستنى نحتفل معاكم — عمر و ريوان';
      ...
      status.textContent = 'مكانكم اتحجز. لو محصلش رد منا، ابعتولنا رسالة على طول.';
      status.style.color = '#667c62';
      form.reset();
      $('#guestCount', form).value = '1';
      $('input[name="attendance"][value="Attending"]', form).checked = true;
      ...
      submitButton.textContent = 'أكدوا حضوركم';
    }
  });
}
...
function initMusic() {
  ...
  btn.addEventListener('click', () => {
    if (playing) {
      ...
      btn.setAttribute('aria-label', 'شغل الموسيقى');
      playing = false;
    } else {
      audio.play()
        .then(() => {
          ...
          btn.setAttribute('aria-label', 'وقف الموسيقى');
        })
        .catch(() => {});
    }
  });
